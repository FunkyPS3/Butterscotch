...
static void loadTextures(SoftRendererInternal *ri, DataWin *dataWin)
{
    ...
    ri->framePageSampleCounts = NULL;
    ri->framePageNearBlackSamples = NULL;
    SoftRenderer_TextureCacheInit();
    SoftRenderer_TextureCacheWarmStart(dataWin);
...
static TexturePageCacheEntry *getTexturePage(SoftRendererInternal *ri, int pageId)
{
    if (!ri)
        return NULL;
    if (pageId < 0 || (uint32_t)pageId >= (uint32_t)ri->textureCount)
        return NULL;
...
static void blitSprite(SoftRendererInternal *ri,
                       ...
                       float alpha)
{
    if (!ri->fb || pageId < 0 || pageId >= ri->textureCount)
        return;
    TexturePageCacheEntry *tex = getTexturePage(ri, pageId);
    if (alpha <= 0.0f || srcW <= 0 || srcH <= 0)
        return;
    if (!tex || !tex->loaded || !tex->pixels_xdr)
        return;
    ...
    if (fabsf(fbDstW) < 0.0001f || fabsf(fbDstH) < 0.0001f)
        return;
                           ...
                           float angle)
{
    ...
    if (vw <= 0)
        vw = (ri->gameWidth > 0) ? ri->gameWidth : 1;
    if (vh <= 0)
        vh = (ri->gameHeight > 0) ? ri->gameHeight : 1;
    if (pw <= 0)
        pw = (ri->gameWidth > 0) ? ri->gameWidth : vw;
    if (ph <= 0)
        ph = (ri->gameHeight > 0) ? ri->gameHeight : vh;
    ...
    if (ri->clipY0 < 0)
        ri->clipY0 = 0;
    if (ri->clipX1 > ri->fbWidth)
        ri->clipX1 = ri->fbWidth;
    if (ri->clipY1 > ri->fbHeight)
        ri->clipY1 = ri->fbHeight;
                               ...
                               uint32_t color, float alpha, bool outline)
{
    ...
    if (outline)
    {
        for (int fx = ix0; fx < ix1; fx++)
        {
            fbPut(ri, fx, iy0, px);
            fbPut(ri, fx, iy1 - 1, px);
        }
        for (int fy = iy0; fy < iy1; fy++)
        {
            fbPut(ri, ix0, fy, px);
            fbPut(ri, ix1 - 1, fy, px);
        }
    }
    else
    {
                          ...
                          float xscale, float yscale, float angleDeg)
{
    SoftRendererInternal *ri = (SoftRendererInternal *)r;
    DataWin *dw = r->dataWin;
    if (!ri->fb || !text || *text == '\0')
        return;
    (void)angleDeg;

    int fontIndex = r->drawFont;
    if (fontIndex < 0 || (uint32_t)fontIndex >= dw->font.count)
        return;
